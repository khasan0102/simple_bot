    ...
    if (message.text == "⬅️ Orqaga") {
      ...
      if (message.media_group_id) {
        ...
        const caption = message.caption
          ? functions.makeResponse(message.caption, message.caption_entities)
          : undefined;
  ...
  try {
    ...
    if (isFirst) {
      bot.sendMessage(query.message.chat.id, responseText, {
        parse_mode: "HTML",
        reply_markup: {
          inline_keyboard: buttons.commentsButtons(comments, 0, count),
        },
      });
    } else {
      bot.editMessageText(responseText, {
        ...
        parse_mode: "HTML",
        reply_markup: {
          inline_keyboard: buttons.commentsButtons(comments, page, count),
        },
      });
    }
  } catch (error) {
...
const getComment = async (bot, query, commentId) => {
    try {
        ...
        bot.sendMessage(query.message.chat.id, `<b>${comment.username}ning izohi</b>\n\n${comment.user_comment}`, {
            parse_mode: "HTML",
            reply_markup: {
